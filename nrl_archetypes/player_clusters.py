...
def load_and_process_data(configs, player_data, stat_mode='production', recent_games=None):
    ...
    # Some match feeds publish null tackle efficiency, impossible zeroes, or
    # fractional positive values such as 0.94 instead of 94, despite supplying
    # the tackle counts.
    ...
    invalid_efficiency = (
        ...
        | ((reported_efficiency == 0) & (tackle_counts['tackles_made'].fillna(0) > 0))
        ...
        for config in configs:
            ...
            fig.update_layout(
                legend=dict(
                    ...
                    y=0,
                    yanchor='bottom',
                    font=dict(color='#f5f7ff', size=14),
                    bgcolor='rgba(0,0,0,0)',
                    borderwidth=0,
                ),
                showlegend=True,
                margin=dict(l=0, r=0, b=58, t=0),
                scene=dict(domain=dict(y=[0.1, 1])),
            ...
            function adjustPlotlyForMobile() {
                ...
                if (window.innerWidth < 768) {
                    const update = {
                        ...
                        showlegend: true,
                        margin: { l: 5, r: 5, b: 58, t: 0 },
                        scene: { ...(gd.layout.scene || {}), domain: { ...((gd.layout.scene || {}).domain || {}), y: [0.1, 1] } },
                        updatemenus: [{
