...
const ALL_PLAYERS_BASE_COLUMNS: Array<{ key: AllPlayersSortKey; label: string; align?: "left" | "center" | "right"; proOnly?: boolean }> = [
  ...
  { key: "tradeWeeklyDelta", label: "Popularity Rating", align: "center", proOnly: true },
  { key: "tradeValue", label: "Value Rating", align: "center", proOnly: true },
  { key: "tradeKeeper", label: "Keeper Rating", align: "center", proOnly: true },
  { key: "tradeRole", label: "Role Rating", align: "center", proOnly: true },
  { key: "tradeForm", label: "Form Rating", align: "center", proOnly: true },
  { key: "tradeBreakeven", label: "Breakeven Rating", align: "center", proOnly: true },
  { key: "tradeAvailability", label: "Availability Rating", align: "center", proOnly: true },
...
const ALL_PLAYERS_MOBILE_SORT_OPTIONS: Array<{ key: AllPlayersSortKey; label: string; proOnly?: boolean }> = [
  ...
  { key: "tradeWeeklyDelta", label: "Popularity Rating", proOnly: true },
  { key: "tradeValue", label: "Value Rating", proOnly: true },
  { key: "tradeKeeper", label: "Keeper Rating", proOnly: true },
  { key: "tradeRole", label: "Role Rating", proOnly: true },
  { key: "tradeForm", label: "Form Rating", proOnly: true },
  { key: "tradeBreakeven", label: "Breakeven Rating", proOnly: true },
  { key: "tradeAvailability", label: "Availability Rating", proOnly: true },
...
export function FantasyDashboard({
 ...
 const rawAllPlayersTableRows = useMemo<AllPlayersTableRow[]>(() => {
  ...
  const availableAllPlayersMobileSortOptions = useMemo(
    () => ALL_PLAYERS_MOBILE_SORT_OPTIONS.map((option) => ({
      ...option,
      label: !hasFantasyPlotAccess && option.proOnly ? `${option.label} (Locked)` : option.label,
    })),
    [hasFantasyPlotAccess]
  )
  const allPlayersSortDisplayColumn: AllPlayersSortKey =
    allPlayersSort.column === "weeklyChange" &&
    sortedAllPlayersTableRows.length > 0 &&
    sortedAllPlayersTableRows.every((row) => row.weeklyChange === null || row.weeklyChange === 0)
      ? "price"
      : allPlayersSort.column
  const selectedAllPlayersMobileSortValue =
    availableAllPlayersMobileSortOptions.find((option) => option.key === allPlayersSortDisplayColumn)?.key ??
    ...
    <div className={showOwnedCards && showFantasyActions && !showAllPlayersOnly && !showFantasyAnalyticsOnly ? "space-y-3" : "space-y-6"}>
      ...
      <div className="space-y-3">
        ...
        {showOwnedCards && showFantasyActions && !showAllPlayersOnly && !showFantasyAnalyticsOnly ? (
          <div className="grid grid-cols-2 gap-2 lg:grid-cols-4">
            ...
            >
              <span className="pointer-events-none absolute inset-0 bg-[linear-gradient(135deg,rgba(123,92,255,0.08),rgba(0,245,138,0.035)_58%,rgba(17,24,50,0)_100%)]" />
              <span className="pointer-events-none absolute inset-0 hidden overflow-hidden rounded-xl lg:block">
            ...
            <div className="contents">
              <div className="contents">
                ...
                <div
                  ...
                  className={`group h-full scroll-mt-24 self-stretch rounded-xl border border-[rgba(123,92,255,0.35)] bg-[#111832] p-0 shadow-[0_8px_16px_rgba(8,10,18,0.16)] transition-colors hover:border-nrl-accent/70 hover:bg-[#17213d] ${
              ...
              {fantasyProjectionArticle ? (
                <Link
                  ...
                  className="group relative flex h-full min-h-[52px] w-full cursor-pointer overflow-hidden rounded-xl border border-[rgba(123,92,255,0.35)] bg-[#111832] text-white shadow-[0_8px_16px_rgba(8,10,18,0.16)] transition-colors hover:border-nrl-accent/70"
                >
                  ...
                  <div className="relative flex h-full min-h-[52px] w-full items-center justify-between gap-2 px-3 py-2 sm:px-4">
                    <div className="min-w-0">
                      <div className="text-[8px] font-bold uppercase tracking-[0.18em] text-nrl-accent">
                        Article
                      </div>
                      <div className="mt-0.5 overflow-hidden text-[8px] font-bold uppercase leading-tight tracking-[0.08em] text-white [display:-webkit-box] [-webkit-box-orient:vertical] [-webkit-line-clamp:1] sm:text-[9px]">
      ...
      {showOwnedCards && !showFantasyAnalyticsOnly ? (
        ...
        >
            ...
            {hasLoadedFullAllPlayersRows ? (
              <div className="flex w-full min-w-0 flex-wrap items-center justify-between gap-2 sm:flex-nowrap sm:gap-1.5">
                <div className="flex w-full min-w-0 flex-wrap items-center gap-1.5 sm:w-auto sm:shrink-0">
                  ...
                  <label className="inline-flex min-h-[30px] shrink-0 cursor-pointer items-center gap-1.5 rounded-full border border-nrl-border bg-nrl-panel-2 px-2 text-[8px] font-bold uppercase tracking-wide text-nrl-muted sm:text-[9px] md:hidden">
                    <span>Tags</span>
                    <input
                      type="checkbox"
          ...
          <div className={`${hasLoadedFullAllPlayersRows ? "block md:hidden" : "hidden"} border-b border-nrl-border bg-nrl-panel px-3 py-2`}>
              ...
              <div className="w-40">
                <Select
                  ...
                  onChange={(sortKey) => {
                    ...
                    if (!option) return
                    if (!hasFantasyPlotAccess && option.proOnly) return
                    setAllPlayersSort((current) => ({
                      column: option.key,
            ...
            {sortedAllPlayersTableRows.length === 0 ? (
              ...
              sortedAllPlayersTableRows.map((row) => {
                ...
                const homePreviewCardStats = [
                  ...
                  .map((key) => baseCardStats.find((stat) => stat.key === key))
                  .filter((stat): stat is (typeof baseCardStats)[number] => Boolean(stat))
                const displayedCardStats = hasLoadedFullAllPlayersRows ? cardStats : homePreviewCardStats
                const showCardStatsStrip = showAllPlayersCardStats || !hasLoadedFullAllPlayersRows
                const selectedCardStat =
                  [...cardStats, ...tradeRatingCardStats].find((stat) => stat.key === allPlayersSortDisplayColumn) ??
                  (allPlayersSortDisplayColumn === "position"
                  ...
                  <button
                    ...
                    style={renderAllPlayersTradeRatings ? ALL_PLAYERS_RATED_CARD_CONTENT_STYLE : ALL_PLAYERS_CARD_CONTENT_STYLE}
                      ...
                      <div className="flex min-w-0 flex-1 items-start gap-2.5">
                        ...
                        {!hasLoadedFullAllPlayersRows ? (
                          <div className="ml-auto hidden w-full max-w-[620px] min-w-0 grid-cols-6 gap-2 md:grid">
                            {homePreviewCardStats.map((stat) => (
                              <div key={`home-inline-${stat.key}`} className="min-w-0">
	                      ...
	                      <div className={`shrink-0 text-right ${hasLoadedFullAllPlayersRows ? "" : "md:hidden"}`}>
                    ...
                    {renderAllPlayersTradeRatings ? (
                      <div className="mt-3 overflow-x-auto rounded-full border border-white/10 bg-white/[0.035] px-3 py-2 [scrollbar-width:none] lg:overflow-visible [&::-webkit-scrollbar]:hidden">
                        <div className="flex min-w-max items-center gap-2.5 lg:grid lg:min-w-0 lg:grid-cols-8 lg:gap-3">
                          {tradeRatingCardStats.map((stat) => {
                            ...
                            return (
                              <div
                                key={stat.key}
                                className={`inline-flex min-h-10 min-w-32 flex-col items-center justify-center rounded-full border px-2.5 lg:w-full lg:min-w-0 ${getTradeScorePillClass(stat.tradeScore, highlighted)}`}
                              >
	                    ...
	                    {showCardStatsStrip && hasLoadedFullAllPlayersRows ? (
                        ...
                        {displayedCardStats.map((stat) => (
                          <div
                            key={stat.key}
	                            className="min-w-[3.75rem] md:min-w-0"
                          >
                            <div className={`text-[8px] font-semibold uppercase tracking-wide ${stat.key === allPlayersSortDisplayColumn ? "text-nrl-accent" : "text-nrl-muted"}`}>
                              {stat.label}
                            </div>
                            {!hasFantasyPlotAccess && stat.locked ? (
                              <div className={`mt-0.5 ${FANTASY_LOCKED_VALUE_BOX_CLASS}`}>
