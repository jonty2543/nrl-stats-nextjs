...
function buildMyTeamAiPrompt({
  ...
  const tradeSuggestorMetricInstructions = hasFantasyPlotAccess
    ? [
      ...
      "Use casualty ward and current Origin lineup context behind ownership, form, value, injury, bye and lineup signals; however, likely Origin players missing multiple major bye rounds should be treated as a major bye-coverage problem, not a minor tie-breaker.",
  ...
  return [
    ...
    hasFantasyPlotAccess
      ? "Sell watch should only include owned players. Prioritise confirmed injury/out/suspension from casualty ward or lineups, current Origin lineup players, live confirmed bye/DNP, high BE, poor projection vs pricedAt, highly traded-out/negative ownership delta, or bad major-bye coverage. If a player is a hold, say hold rather than forcing a sale."
...
function canSwapMyTeamPlayers(
  team: SavedMyTeam,
  sourceIndex: number,
  targetIndex: number,
  fantasyPlayersById: Map<number, FantasyPlayerSnapshot>,
): boolean {
  const targetPlayer = team.players[targetIndex] ?? null
  if (!targetPlayer) return false
  const result = swapMyTeamPlayers(team, sourceIndex, dropTargetForPlayer(targetPlayer, targetIndex), fantasyPlayersById)
  return result.error == null && result.team != null
}
...
export function MyTeamPage({ fantasyPlayers, fantasyCoachPlayers, lineupsProjections, playerImages, draw2026Data, originChances, locked }: MyTeamPageProps) {
  ...
  useEffect(() => {
    ...
    const handlePointerDown = (event: PointerEvent) => {
      ...
      if (isSwapMenuOpen && teamBoardRef.current?.contains(event.target as Node)) return
  ...
  }, [isSwapMenuOpen, selectedPlayerIndex])
