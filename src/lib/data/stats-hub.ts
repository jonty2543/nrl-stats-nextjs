...
function minimumGameMinutesForPosition(position: string | null | undefined): number {
  return Math.max(18, Math.round(minimumAverageMinutesForPosition(position) * 0.8));
}

function hasUsualMinutes(row: PlayerStat): boolean {
  return numeric(row["Mins Played"]) >= minimumGameMinutesForPosition(row.Position);
}

function eligiblePriorRows(row: PlayerStat, playerGroups: Map<string, PlayerStat[]>): PlayerStat[] {
  const priorRows = (playerGroups.get(playerKey(row)) ?? []).filter((candidate) => candidate.Round < row.Round && hasUsualMinutes(candidate));
...
function buildOutlierInsights(latestRows: PlayerStat[], playerGroups: Map<string, PlayerStat[]>): StatsHubInsight[] {
  ...
  latestRows.forEach((row) => {
    ...
    PLAYER_OUTLIER_STATS.forEach((stat) => {
      ...
      const threshold = Math.max(4, baseline * 0.25);
      if (Math.abs(diff) <= threshold) return;
      const isDrop = diff < 0;

      insights.push({
        id: insightId(isDrop ? "outlier-drop" : "outlier-surge", [row.Name, stat.key, row.Year, row.Round]),
        category: "Player Stat Outliers",
        title: row.Name,
        detail: "",
        score: Math.round(Math.abs(diff)),
        ...
        visualMode: "metric",
        metrics: [
          { label: "Last Week", value: `${formatNumber(value)} ${stat.label}`, tone: isDrop ? "down" : "up" },
          { label: "Season Prior Average", value: `${formatNumber(baseline)} ${stat.label}` },
          { label: "Difference", value: `${diff >= 0 ? "+" : ""}${formatNumber(diff)} ${stat.label}`, tone: isDrop ? "down" : "up" },
        ],
      });
    });
  });
...
function buildDueInsights(latestRows: PlayerStat[], playerGroups: Map<string, PlayerStat[]>): StatsHubInsight[] {
  ...
  latestRows.forEach((latestRow) => {
    if (!hasUsualMinutes(latestRow)) return;
    ...
    const countDryRounds = (stat: "Tries" | "Line Breaks" | "Fantasy", threshold: number) => {
      ...
      for (let index = rows.length - 1; index >= 0; index -= 1) {
        if (!hasUsualMinutes(rows[index])) continue;
    ...
    if (tryAverage != null && tryAverage >= 0.15 && tryDryRounds >= 3) {
      insights.push({
        ...
        metrics: [
          ...
          { label: "Current Drought", value: `${tryDryRounds} game drought`, tone: "down" },
        ],
      });
    }
    ...
    if (linebreakAverage != null && linebreakAverage >= 0.12 && linebreakDryRounds >= 3) {
      insights.push({
        ...
        metrics: [
          ...
          { label: "Current Drought", value: `${linebreakDryRounds} game drought`, tone: "down" },
        ],
      });
    }
    ...
    if (fantasyAverage != null && fantasyAverage >= 38 && fantasyDryRounds >= 2) {
      insights.push({
        ...
        visualMode: "metric",
        metrics: [
          { label: "Average Fantasy", value: `${formatNumber(fantasyAverage)} fantasy` },
          { label: "Latest Fantasy", value: `${formatNumber(numeric(latestRow.Fantasy))} fantasy`, tone: "down" },
          { label: "Fantasy Gap", value: `${formatNumber(fantasyAverage - numeric(latestRow.Fantasy))} below average`, tone: "down" },
        ],
      });
    }
  });
  ...
  return specs.flatMap((spec) => {
    const candidates = latestRows
      .map((row) => {
        ...
        const priorRows = rows.slice(0, Math.max(0, rowIndex - 2)).filter(hasUsualMinutes);
        if (recentRows.length < 3 || priorRows.length < MIN_PRIOR_GAMES) return null;
        if (recentRows.some((candidate) => !hasUsualMinutes(candidate))) return null;
        const averageMinutes = average(priorRows.map((candidate) => numeric(candidate["Mins Played"]))) ?? 0;
    ...
    const selected = [
      ...candidates.filter((candidate) => candidate.diff > 0).sort((a, b) => b.diff - a.diff).slice(0, spec.limit),
      ...candidates.filter((candidate) => candidate.diff < 0).sort((a, b) => a.diff - b.diff).slice(0, spec.limit),
    ].sort((a, b) => Math.abs(b.diff) - Math.abs(a.diff));

    return selected.map(({ row, recentAverage, priorAverage, diff }) => {
      const isDrop = diff < 0;
      return {
        id: insightId(isDrop ? "player-trend-drop" : "player-trend-surge", [row.Name, spec.key, row.Year, row.Round]),
        category: "Player Stat Outliers" as const,
        title: `${row.Name}: ${spec.label} ${isDrop ? "drop" : "surge"}`,
        detail: "",
        score: Math.abs(diff),
        ...
        metrics: [
          { label: "Last 3 Avg", value: `${formatNumber(recentAverage)} ${spec.label}`, tone: isDrop ? "down" as const : "up" as const },
          { label: "Season Prior Avg", value: `${formatNumber(priorAverage)} ${spec.label}` },
          { label: "Difference", value: `${diff >= 0 ? "+" : ""}${formatNumber(diff)} ${spec.label}`, tone: isDrop ? "down" as const : "up" as const },
        ],
      };
    });
  });
  ...
  return specs.flatMap((spec) => {
    ...
    return candidates.map(({ row, recentAverage, priorAverage, diff }) => {
      ...
      return {
        ...
        category: "Team Stat Outliers" as const,
...
function categoryGroups(insights: StatsHubInsight[]): StatsHubModel["categories"] {
  const playerOutlierRank = (insight: StatsHubInsight) => {
    if (insight.id.startsWith("player-trend-surge")) return 0;
    if (insight.id.startsWith("outlier-surge") || insight.id.startsWith("outlier-")) return 1;
    if (insight.id.startsWith("player-trend-drop")) return 2;
    return 3;
  };

  return CATEGORY_ORDER.map((category) => {
    const limit = category === "Interesting Facts" ? 10 : 12;
    if (category === "Player Stat Outliers") {
      const playerInsights = insights.filter((insight) => insight.category === category);
      const movementInsights = playerInsights
        .filter((insight) => !insight.id.startsWith("due-"))
        .sort((a, b) => {
          const rankDiff = playerOutlierRank(a) - playerOutlierRank(b);
          if (rankDiff !== 0) return rankDiff;
          return b.score - a.score;
        })
        .slice(0, 6);
      const dueInsights = playerInsights
        .filter((insight) => insight.id.startsWith("due-"))
        .sort((a, b) => b.score - a.score)
        .slice(0, 6);
      const mixedInsights = Array.from({ length: Math.max(movementInsights.length, dueInsights.length) })
        .flatMap((_, index) => [movementInsights[index], dueInsights[index]])
        .filter((insight): insight is StatsHubInsight => Boolean(insight));
    ...
    return {
      category,
      insights: categoryInsights.slice(0, limit),
    };
